from _md_entities import db, Ocorrencia, ControleDeExcecoes
from _md_logger import Logger
from datetime import datetime


class NenhumValorEncontrado(Exception):
    def __init__(self):
        super().__init__('NenhumValorEncontrado')
...
def criar_ocorrencia(id_aluno: int, id_serie: str, oficina: str, assunto: str,
    ...
    try:
        ...
        raise ControleDeExcecoes(f'Erro ao criar ocorrência para o aluno ({id_aluno})', e)
    finally:
        return False, None
...
def ler_ocorrencia(_id: int = None, _serie: str = None, _aluno: int = None) -> tuple[bool, list[Ocorrencia] | None]:
    ...
    try:
        with db:
            ...
            raise NenhumValorEncontrado
    except Exception as e:
        Logger.error(f'Erro ao ler ocorrência: {e}')
        raise ControleDeExcecoes('Erro ao ler ocorrência', e)
    finally:
        return False, None
